...
- **Gate each report mechanically before trusting the diff**: `codex 호출 0회`, a missing session id, or no `OpenAI Codex v` banner in `out.txt` → the change did not come from codex, re-dispatch; non-zero `검증 … exit` → send the failing lines back as rework; non-empty `스펙 외 변경 파일` → inspect those paths. Read the report + `git diff --stat` + `last.txt` first; open the full diff only on failure, out-of-scope files, or the risk surface (`rules/risk-triage.md` requires a reviewer there anyway — the worker's report never substitutes for it).
